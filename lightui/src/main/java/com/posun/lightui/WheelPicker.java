...
import com.posun.lightui.citypicker.IDebug;
import com.posun.lightui.citypicker.IWheelPicker;
...
public class WheelPicker extends View implements IDebug, IWheelPicker, Runnable {
     ...
     */
    private int mVisibleItemCount=7, mDrawnItemCount;
     ...
     */
    private int mItemTextColor= Color.BLACK, mSelectedItemTextColor= Color.RED;
     ...
     */
    private int mTextMaxWidthPosition=-1;
     ...
     */
    private boolean hasSameWidth=false;
    ...
    public WheelPicker(Context context, int color) {
        super(context);
        this.mSelectedItemTextColor=color;
        initWheel();
    }
    public WheelPicker(Context context, int color, int itemsize) {
        super(context);
        this.mItemTextSize=itemsize;
        this.mSelectedItemTextColor=color;
        initWheel();
    }
    ...
    public void initWheel() {
        mData = Arrays.asList(new String[]{"item","item","item","item","item","item","item","item"});
        updateVisibleItemCount();
        if(mItemTextSize==-1){
            mItemTextSize= QlightUnit.sp2px(getContext(),20);
        }else{
            mItemTextSize= QlightUnit.sp2px(getContext(),mItemTextSize);
        }
...
//        invalidate();
    ...
    public void setSelectedItemPosition(String position) {
        int size=mData.size();
        for(int i=0;i<size;i++){
            if(position.equalsIgnoreCase(String.valueOf(mData.get(i)))){
    ...
    public String getCurrentItemString() {
        return String.valueOf(mData.get(mCurrentItemPosition));
    }
    @Override
    public List getData() {
        return mData;
