...
public class LightDefultLayoutManager extends RecyclerView.LayoutManager {
    ...
    public LightGroupRecycler getGroupRecycler() {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (!groupRecycler.haveGroup()) {
            isGroupAdapter = false;
        } else if (isGroupAdapter && suspensionmanager == null) {
            suspensionmanager = new SuspensionManager();
            suspensionmanager.setRecyclerView(groupRecycler.getRecyclerView());
        }
        ...
        if (position != 0) {
            ...
            groupRecycler.removeItemOtherLineSpan();
        }
    ...
    private int fillLayout(int dy, RecyclerView.Recycler recycler) {
        ...
        if (dy > 0) {
            ...
            if (isGroupAdapter && groupRecycler.isGroupView(lastView)) {
                layoutState.position.setLockshowGroup(true);
            }
    ...
    private int juageHasGroupView(View fistview, int offsetY, int position) {
        int addheight = 0;
        if (isGroupAdapter && groupRecycler.isGroup(position)) {
    ...
    private void addAfterItem(View view, LightLayoutState state) {
        ...
        int linespan = groupRecycler.getItemOtherLineSpan(state.position.getIndex());
        if (linespan != LightGroupRecycler.NOVALUE) {
            state.afterSpan = spanCount - linespan;
        }
    ...
    private int tryAddafterView(View view, LightLayoutState state, int span) {
        ...
        Rect rect = new Rect(left, state.offsetY.getIndex() - height, right, state.offsetY.getIndex());
        addViewToRecycleView(view, 0, false, rect);
        return height;
    }
    ...
    public void addViewToRecycleView(View view, int position, boolean isGroup, Rect rect) {
        groupRecycler.addViewToRecycleView(view, position, this, isGroup);
        layoutDecoratedWithMargins(view, rect.left, rect.top, rect.right, rect.bottom);
    }

    public void addViewToRecycleView(View view, boolean isGroup, Rect rect) {
        addViewToRecycleView(view, -1, isGroup, rect);
    }
    ...
    static class ItemView {
        ...
        public ItemView(View view, int position, boolean isGroup, Rect rect) {
            this.rect = rect;
            this.view = view;
    ...
    private int tryAddBeforView(View view, LightLayoutState state, int span) {
        ...
        Rect rect = new Rect(left, state.offsetY.getIndex(), right, state.offsetY.getIndex() + height);
        addViewToRecycleView(view, false, rect);
        return height;
    }

    public void addViewToNextLine(LightLayoutState state) {
        groupRecycler.setItemOtherLineSpan(state.position.getIndex() - 1, state.beforSpan);
    ...
    private class LightLayoutState {
        int beforSpan = 0, afterSpan = 0, MaxItemHeight = Integer.MIN_VALUE;
        ...
        private void clearItem() {
            MaxItemHeight = Integer.MIN_VALUE;
        }
        ...
        public View getNextView() {
            ...
            if (isGroupAdapter && groupRecycler.getGroupAdapter(position.getIndex()).isGroup(position.getIndex()) && !position.isLockshowGroup()) {
        ...
        public View getPreviousView() {
            ...
            if (!position.isLockshowGroup()) {//解锁状态时正常倒序渲染item
                view = recycler.getViewForPosition(position.getIndex());
                if (isGroupAdapter && groupRecycler.getGroupAdapter(position.getIndex()).isGroup(position.getIndex())) {
