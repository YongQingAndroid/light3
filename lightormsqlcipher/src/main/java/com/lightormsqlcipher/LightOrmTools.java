...
public final class LightOrmTools {
    ...
    public void remove(String table_name, WhereBulider whereBulider) {
        ...
        try {
            sqLiteDatabase = getWritableDatabase(sqLiteOpenHelperWeakReference.get());
    ...
    * */
    ...
    private SQLiteDatabase getWritableDatabase(SQLiteOpenHelper sqLiteOpenHelper){
             return sqLiteOpenHelper.getWritableDatabase(DBHelper.DB_PWD);
    }
    private SQLiteDatabase getReadableDatabase(SQLiteOpenHelper sqLiteOpenHelper){
        return sqLiteOpenHelper.getReadableDatabase(DBHelper.DB_PWD);
    }
    ...
    public <T> List<T> query(Class<T> clazz, String tableName, WhereBulider whereBulider) {
        ...
        SQLiteDatabase sqLiteDatabase = getReadableDatabase(sqLiteOpenHelperWeakReference.get());
        if (!havetable(tableName, sqLiteDatabase)) {
            LightLog.e(tableName + " Table does not exist");
            return null;
        }
     ...
     * @param whereBulider 条件
     */
    public String query(String tableName,WhereBulider whereBulider) {
        if (sqLiteOpenHelperWeakReference.get() == null) {
            LightLog.e(tableName + " query file error");
        }
        SQLiteDatabase sqLiteDatabase = getReadableDatabase(sqLiteOpenHelperWeakReference.get());
