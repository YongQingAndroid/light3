...
import com.lightormsqlcipher.maping.MapingBean;
import com.lightormsqlcipher.maping.MappingArrayList;
...
/**
 * 对象级数据库，面向对象的数据库存储 (支持sqlcipher AES-256加密)
 * ORM加密数据库仅供学习和体验请勿商用加密代码归（sqlcipher）所有
 * 支持迁移数据库到SDCard
 * 自动侦测数据库表字段变化自动升级数据库
 * 查询数据时带有映射关系类型必须为ArrayList或List
 * 优化映射关系数据为异步查询，返回集合对象使用到映射数据时才会触发数据库条件搜索
 * 映射子查询不可用于兼容老数据库表
 * 作者：zyq on 2017/11/9 15:17
 * 邮箱：zyq@posun.com
 * @author zyq
 */
...
//    }
    ...
    private void openDataBase() {
        ...
        sqLiteDatabase = helper.getWritableDatabase(DBHelper.DB_PWD);
    }
    /**
     * 打开只读数据库
     * */
    private void openOnlyReadDataBase() {
        if (helper == null)
            helper = new DBHelper(getContext());
        sqLiteDatabase = helper.getReadableDatabase(DBHelper.DB_PWD);
    }
    ...
    public <M> List<M> query(Class<M> mClass, String tableName, WhereBulider whereBulider) {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                Object item = mClass.newInstance();
